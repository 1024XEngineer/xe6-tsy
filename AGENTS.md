...
- Codex 可以在本地执行只读检查、代码编辑和验证，但不得自行创建或修改 GitHub Issue、
  Pull Request、评论、Review、Milestone、Release、标签或其他远端仓库内容。
- 任何 `git commit`、`git push`、分支发布、PR/Issue 创建或修改，都必须先向用户展示拟提交
  的文件、差异摘要、提交信息、目标分支，以及拟发布的标题和正文（如适用）。
- 只有用户在看到上述拟执行内容后作出明确、针对本次操作的真实确认，Codex 才能执行；此前
  的笼统授权、要求“完成任务”或自动继续指令不视为提交和远端操作授权。
- 一次确认只授权当次明确列出的操作。内容、文件范围、目标分支或远端正文发生变化时，必须
  重新展示并取得确认，不得沿用旧确认。
- 未取得确认时，所有改动保持在本地未提交状态，不得通过 amend、force push、API、CLI、
  自动化脚本或其他方式绕过本门禁。
