...
These rules apply to `apps/api` and its subdirectories. They extend the repository-level
`AGENTS.md` and the unified development spec.

## 后端 Go 注释与风格

- 后端复杂逻辑必须配英文解释型注释，尤其是状态迁移、重试恢复、授权审计、并发控制、
...
- 涉及正式请求、响应、通知、错误对象、事件或交付对象时，先对齐 `../../packages/contracts`
