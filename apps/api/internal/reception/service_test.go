...
func newTestSystem() *testSystem {
	...
	service := NewService(Deps{
		Store: store, Authorizer: authorizer, Configs: &FakeOrganizationConfigReader{},
		Processing: processing, Media: media, Cleaner: cleaner,
		Clock: &FixedClock{NowValue: time.Date(2026, 7, 15, 8, 0, 0, 0, time.UTC)},
...
func TestEndWithActiveBindingWritesCleanupEvent(t *testing.T) {
	...
	binding, _ := system.store.GetBinding(context.Background(), attached.Binding.BindingID)
	if binding.Status != MediaTrackDetached || system.cleaner.Count() != 1 {
		t.Fatalf("binding = %q, cleaner = %d", binding.Status, system.cleaner.Count())
	}
	events := system.store.Events()
	if events[len(events)-2].EventType != EventMediaTrackDetached || events[len(events)-1].EventType != EventReceptionSessionEnded || events[len(events)-1].AggregateType != "ReceptionSession" {
		t.Fatalf("cleanup events = %#v", events[len(events)-2:])
	}
}

func TestEndWithDetachFailureKeepsStateRetryable(t *testing.T) {
	system := newTestSystem()
	started := startSession(t, system, createSession(t, system, "create-end-df"), "start-end-df")
	attached, _ := attachMedia(t, system, started.Session, "attach-end-df", FakeScenarioDetachFailure)

	_, err := system.service.EndSession(context.Background(), EndReceptionSessionCommand{
		SessionID: started.Session.SessionID, AccessContextRef: "access-demo",
		ExpectedVersion: started.Session.Version, IdempotencyKey: "end-df",
	})
	if errorCode(t, err) != CodeMediaDetachFailed {
	...
	session, _ := system.store.GetSession(context.Background(), started.Session.SessionID)
	binding, _ := system.store.GetBinding(context.Background(), attached.Binding.BindingID)
	if session.Status != ReceptionSessionActive || binding.Status != MediaTrackAttached {
		t.Fatalf("session = %q, binding = %q", session.Status, binding.Status)
	}
}

func TestCancelRetriesAfterCleanupFailure(t *testing.T) {
	system := newTestSystem()
	started := startSession(t, system, createSession(t, system, "create-cancel-clean"), "start-cancel-clean")
	attached, _ := attachMedia(t, system, started.Session, "attach-cancel-clean", FakeScenarioSuccess)
	command := CancelReceptionSessionCommand{
		SessionID: started.Session.SessionID, AccessContextRef: "access-demo",
		ExpectedVersion: started.Session.Version, IdempotencyKey: "cancel-clean",
		ReasonCode: "operator_cancelled",
	}
	system.cleaner.Fail = true
	if _, err := system.service.CancelSession(context.Background(), command); errorCode(t, err) != CodeMediaDetachFailed {
	...
	system.cleaner.Fail = false
	cancelled, err := system.service.CancelSession(context.Background(), command)
	if err != nil || cancelled.Status != ReceptionSessionCancelled {
		t.Fatalf("CancelSession() = %#v, %v", cancelled, err)
	}
	binding, _ = system.store.GetBinding(context.Background(), attached.Binding.BindingID)
	if binding.Status != MediaTrackDetached {
		t.Fatalf("binding = %q", binding.Status)
	}
}
