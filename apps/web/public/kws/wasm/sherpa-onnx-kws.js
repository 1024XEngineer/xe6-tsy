  ...
  SherpaOnnx.KWS = {
    ...
    loadModel: async function(modelConfig) {
      ...
      try {
        ...
        // Emscripten FS uses errno=20 for EEXIST (differs from POSIX errno=17)
    ...
    createKeywordSpotter: function(loadedModel, options = {}) {
      ...
      // Try to read keywords from file loaded by loadModel, fall back to options or defaults
      let loadedKeywordsContent = null;
      if (loadedModel.paths.keywordsFile) {
        try {
          loadedKeywordsContent = global.Module.FS.readFile(loadedModel.paths.keywordsFile, { encoding: 'utf8' });
          if (debug) console.log(`Read keywords from loaded file: ${loadedModel.paths.keywordsFile} (${loadedKeywordsContent.length} bytes)`);
        } catch (e) {
          if (debug) console.warn(`Failed to read loaded keywords file: ${e.message}`);
        }
      }

      // Use: 1) explicit options.keywords, 2) loaded file content, 3) hardcoded defaults
      let defaultKeywords =
        "▁HE LL O ▁WORLD\n" +
        "▁HI ▁GO O G LE\n" +
        "▁HE Y ▁S I RI";

      let keywordsContent = options.keywords || loadedKeywordsContent || defaultKeywords;
      ...
      if (typeof createKws === 'function') {
        kws = createKws(global.Module, configObj);
      } else {
        kws = new global.Kws(configObj, global.Module);
      }
      return kws;
    }
  };
...
function initModelConfig(config, Module) {
  ...
  // C struct SherpaOnnxOnlineModelConfig: transducer(12) + paraformer(8) + ctc(4) +
  // 9 scalar/ptr fields(36) + nemo_ctc(4) + t_one_ctc(4) = 68 bytes
  const nemoCtcLen = 1 * 4;
  const toneCtcLen = 1 * 4;
  const len = transducer.len + paraformer_len + ctc_len + 9 * 4 + nemoCtcLen + toneCtcLen;
  const ptr = Module._malloc(len);
  ...
  // Allocate string buffer for all string fields (must not be NULL — C code may strlen without null check)
  ...
  // Write struct fields at correct offsets
  offset = transducer.len + paraformer_len + ctc_len;  // offset 24
  Module.setValue(ptr + offset, buffer, 'i8*');  // tokens
  offset += 4;

  Module.setValue(ptr + offset, config.numThreads || 1, 'i32');  // num_threads
  offset += 4;

  Module.setValue(ptr + offset, buffer + tokensLen, 'i8*');  // provider
  offset += 4;

  Module.setValue(ptr + offset, config.debug, 'i32');  // debug
  offset += 4;

  Module.setValue(ptr + offset, buffer + tokensLen + providerLen, 'i8*');  // model_type
  offset += 4;

  Module.setValue(ptr + offset, buffer + tokensLen + providerLen + modelTypeLen, 'i8*');  // modeling_unit
  offset += 4;

  Module.setValue(ptr + offset, buffer + tokensLen + providerLen + modelTypeLen + modelingUnitLen, 'i8*');  // bpe_vocab
  offset += 4;

  Module.setValue(ptr + offset, buffer + tokensLen + providerLen + modelTypeLen + modelingUnitLen + bpeVocabLen, 'i8*');  // tokens_buf
  offset += 4;

  Module.setValue(ptr + offset, config.tokensBufSize || 0, 'i32');  // tokens_buf_size
...
function initKwsConfig(config, Module) {
  ...
  // Prefer inline keywords_buf (C API requires buf_size > 0). Fall back to
  // keywords_file path in the Emscripten MEMFS.
  let keywordsFileBuffer = 0;
  let keywordsBufPtr = 0;
  let keywordsBufSize = 0;

  if (config.keywords && String(config.keywords).length > 0) {
    const text = String(config.keywords);
    keywordsBufSize = Module.lengthBytesUTF8(text);
    const alloc = keywordsBufSize + 1;
    keywordsBufPtr = Module._malloc(alloc);
    Module.stringToUTF8(text, keywordsBufPtr, alloc);
  ...
  // keywords_buf
  Module.setValue(ptr + offset, keywordsBufPtr, 'i8*');
  offset += 4;

  // keywords_buf_size
  Module.setValue(ptr + offset, keywordsBufSize, 'i32');
  offset += 4;
...
function freeConfig(config, Module) {
  ...
  if ('buffer' in config && config.buffer) {
  ...
  if ('keywordsBufPtr' in config && config.keywordsBufPtr) {
    Module._free(config.keywordsBufPtr);
  }

  if ('keywordsBuffer' in config && config.keywordsBuffer) {
...
// Explicitly publish factories on window. Top-level function declarations are
// not always visible as window.createKws depending on how the script is served.
if (typeof window !== 'undefined') {
  window.createKws = createKws;
  if (typeof window.Kws !== 'function' && typeof Kws === 'function') {
    window.Kws = Kws;
  }
}
