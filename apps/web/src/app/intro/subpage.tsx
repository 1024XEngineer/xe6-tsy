...
export function ProductPage() {
  return (
    <SubpageFrame>
      ...
      <Reveal><section className={styles.detailSectionAlt}><div className={styles.detailSectionHeader}><p className={styles.sectionEyebrow}>04 / OUTPUT</p><h2>把结果送达，<br /><span>不把原始音频留下。</span></h2><p>文本 Final Turn 和 UsageFact 是长期事实；P0 不持久化原始音频，也不把字幕当成主链路。</p></div><div className={styles.outputGrid}><OutputItem title="双向听译" copy="两个语言方向都完成 ASR、翻译、FinalTurn 保存和目标语言 TTS。" /><OutputItem title="单向投递" copy="一侧播放译音，另一侧的有效 FinalTurn 异步发送到已绑定的企业微信目标。" /><OutputItem title="可选展示" copy="有屏幕时展示原文、译文和状态；没有屏幕也能通过音频完成交流。" /></div></section></Reveal>
