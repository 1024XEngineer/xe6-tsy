...
flowchart LR
    ...
    Client -->|"登录、会话、语言设置、临时通行证"| Gateway
    Gateway --> API1
    Gateway --> API2
    API1 --> Storage
    API2 --> Storage
    API1 -->|"分配节点、开始、结束、查询状态"| RT1
    API2 -->|"分配节点、开始、结束、查询状态"| RT2
...
业务管理服务是**一个统一入口**，不等于只能部署一台机器。它处理登录、会话、设置和查询，这些请求持续时间短，重要数据都保存在数据库和任务队列中。因此一台就能管理较多会话，请求增多后也可以复制成多台。

实时语音服务不同：每一场正在进行的对话都会长时间占用一条语音连接、音频缓冲、外部识别连接、处理器、内存和网络，所以它主要根据**同时在线的会话数量**增加机器。

一场会话开始时，系统先选一台空闲的实时语音节点。这场会话之后的声音、状态查询和结束请求都回到同一台节点，不能把同一场对话的声音随机发给不同机器：

```text
业务管理服务创建会话
 -> 根据节点是否健康、是否有空位和所在地区选择实时语音节点
 -> 签发只允许进入这场会话、短时间有效的通行证
 -> 客户端连接指定节点
 -> 这场会话的实时处理和语音连接都留在该节点
 -> 会话结束后释放连接和机器资源
```
...
2. **多台实时节点如何分工**：需要知道每台节点是否健康、还有多少空位，并记录每场会话被分到哪台机器。
3. **实时节点突然故障怎么办**：需要明确如何发现失联、回收孤立连接，以及一场进行中的对话能否恢复或只能重连。
...
8. **实时通行证安全**：需要补齐主动作废、会话结束后拒绝旧通行证、定期更换签名密钥和防止重复使用。
