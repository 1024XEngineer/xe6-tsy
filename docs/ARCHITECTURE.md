...
```text
终端认证
 -> API 创建 Session 并校验账户归属
 -> 写入或读取语言配置
 -> API 分配 realtime 节点并签发短期 ticket
 -> 终端与目标节点完成 WebRTC 信令
 -> WebRTC connected 后启动媒体 Pipeline
```

普通 Access Token 只用于 API；realtime ticket 只允许访问指定 Session 和节点的实时资源，避免长期账户凭证进入媒体链路。
...
WebRTC audio track
 ...
 -> ASR partial / final
 -> 翻译与上下文纠偏
 -> 生成 FinalTurn 和 UsageFact
 -> TTS 与播放指令
 -> API 幂等保存 VoiceTurn、汇总用量
```

partial 只服务实时反馈和纠偏；只有句末确认后的 final 译文可以进入 TTS 和长期记录。记录、用量或消息依赖短暂失败时通过可靠事件重试，不阻塞下一轮实时交互。

### 7.3 打断播放

TTS 播放期间检测到新发言时，realtime-audio 停止旧播放、取消旧的待播放工作并返回听音状态。播放决策完全属于媒体面，不写入 Session 业务状态。
