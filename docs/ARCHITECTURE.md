...
Lingow 首期解决两方、双语、面对面的句级语音传译：用户轮流说话，每个语言方向可以选择把译文显示在用户终端，或在一句话结束后播放给对方；需要播放时支持抢话和打断。
...
| 语言配置 | API | 支持语言、双语配置、分方向输出方式、版本与快照 | 执行 ASR 或翻译 |
...
erDiagram
    ...
    LanguageConfigSnapshot {
        ...
        string output_policy_by_direction
    }
    Participant {
        string id
        string session_id
...
- `LanguageConfigSnapshot` 在每个 Turn 开始时读取一次，同时固定本方向的译文是显示到终端还是播放给对方；本轮内保持不变，避免处理中途改变翻译语义或输出目标。
...
从用户视角看，主流程只有“开始、说话、选择译文发到哪里、可以插话、结束和查看记录”；控制面和媒体面的拆分，是为了让这些简单操作在失败、重试和扩容时仍然保持一致。
...
| 句级 Turn、按方向选择字幕或句末播音 | 首期语义稳定；不需要发声的方向跳过语音生成和播放，需要发声的方向仍有清晰的回声与打断边界 | 边听边播和多人会议同传需要更复杂的增量语义修正与音频混流 |
