...
Start/stop replay records reserve their key before invoking the lifecycle operation, so concurrent identical requests share one in-flight result and concurrent payload changes are rejected. Only successful results are retained, for a bounded ten-minute replay window and up to 4,096 records per handler; expired records are purged and a full cache returns `503 replay_capacity_exhausted` until capacity is available.
