...
func TestFinalTurnReaderDeliversFrozenSnapshotsThroughDelivery(t *testing.T) {
	...
	codec, err := NewCursorCodec([]byte("snapshot-delivery-integration-key"))
	if err != nil {
	...
	reader := delivery.NewRecordsTurnReader(turns.NewFinalTurnReader(repository))
	snapshots, err := reader.ReadFinalTurns(t.Context(), "account_01", []string{"snapshot_02", "snapshot_01"})
	if err != nil {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			snapshots, err := reader.ReadFinalTurns(t.Context(), "account_01", test.ids)
