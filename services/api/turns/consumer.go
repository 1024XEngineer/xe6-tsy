...
// maxFinalTurnAttempts bounds transient retries before a durable event is dead-lettered for
// operator inspection and explicit replay.
const maxFinalTurnAttempts = 8
...
type FinalTurnDelivery interface {
	...
	Attempts() int
	Ack() error
	Nack(lastError string) error
	Reject(lastError string) error
}
...
func (h *FinalTurnHandler) Handle(ctx context.Context, delivery FinalTurnDelivery) error {
	if err := h.consumer.ConsumeFinalTurn(ctx, delivery.Event()); err != nil {
		if isPermanentFinalTurnError(err) {
			if rejectErr := delivery.Reject(err.Error()); rejectErr != nil {
		...
		if delivery.Attempts() >= maxFinalTurnAttempts {
			if rejectErr := delivery.Reject(err.Error()); rejectErr != nil {
				return fmt.Errorf("%w: reject exhausted final turn: %w", ErrFinalTurnSettlement, errors.Join(err, rejectErr))
			}
			return fmt.Errorf("reject exhausted final turn: %w", err)
		}
		if nackErr := delivery.Nack(err.Error()); nackErr != nil {
...
func isPermanentFinalTurnError(err error) bool {
	return errors.Is(err, ErrInvalidRequest) || errors.Is(err, domain.ErrConflict) || errors.Is(err, domain.ErrNotFound)
}
