...
- `translate/qwen` uses the OpenAI-compatible `POST /chat/completions` endpoint with `qwen3.6-flash`. Thinking is disabled by default for turn-level latency. User content nests the source sentence inside a language-aware “translate this sentence” frame; meta-refusals trigger one reinforced retry, then persist `realtime_translation_rejected` instead of a generic pipeline failure.
