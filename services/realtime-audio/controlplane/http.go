...
// ConnectionReader returns the current WebRTC connection snapshot without
// exposing the connection manager's mutation capabilities.
...
func (h *Handler) stop(writer http.ResponseWriter, request *http.Request) {
	...
	h.handleReplay(writer, request.Context(), sessionID, "stop\x00"+sessionID+"\x00"+idempotencyKey, body, func() (any, error) {
		return h.lifecycle.Stop(request.Context(), session.StopRealtimeCommand{
			SessionID: sessionID, TraceID: body.TraceID, Reason: body.Reason,
...
func (h *Handler) connection(writer http.ResponseWriter, request *http.Request) {
	...
	if snapshot.SessionID != sessionID {
		h.writeError(writer, request, webrtc.ErrConnectionSessionMismatch)
		return
	}
	h.writeJSON(writer, http.StatusOK, snapshot)
}
...
func mapError(err error) (int, string) {
	...
	case errors.Is(err, session.ErrRuntimeCleanupRequired), errors.Is(err, session.ErrSessionNotCreated),
		...
		errors.Is(err, webrtc.ErrConnectionSessionMismatch), errors.Is(err, ErrConfigSession):
